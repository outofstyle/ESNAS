...
class RBF:
    ...
    def fit(self, train_data, train_label):
        ...
        self.model = RBFInterpolant(
            dim=train_data.shape[1], kernel=kernel(), tail=tail(train_data.shape[1]), lb=0, ub=1)
...
class RBFEnsemble:
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_models):
            ...
            rbf = RBFInterpolant(
                dim=n_feature, kernel=CubicKernel(), tail=LinearTail(n_feature), lb=0, ub=1)
            for i in range(len(X)):
                rbf.add_points(X[i, selected_feature_ids], y[i])
            # rbf.add_points(X[:, selected_feature_ids], y)
